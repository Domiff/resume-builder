...
@router.post("/resume")
async def create_resume(request: HttpRequest, resume_schema: ResumeIn):
    """Create a new resume."""
...
@router.get("/resume")
async def get_resume(request: HttpRequest) -> list[ResumeOut]:
    """Get all resumes."""
    resume_orm = await aget_list_or_404(Resume)
    resume_schemas = [ResumeOut.from_orm(resume) for resume in resume_orm]
    return resume_schemas


@router.get("/resume/{resume_id}")
async def get_resume_by_id(request: HttpRequest, resume_id: int) -> ResumeOut:
    """Get a resume by id."""
